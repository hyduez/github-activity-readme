import { BaseUrl, Emojis, type Item } from "../config";

export class Formatter {
	public EventsSerials: Record<string, (item: Item) => string> = {
		CommitCommentEvent: (item) => {
			const { x: origin, y: commit } = this.parseLink(item) as {
				x: string;
				y: string;
			};
		...
		IssueCommentEvent: (item) => {
			const { x: origin, y: issue } = this.parseLink(item) as {
				x: string;
				y: string;
			};
			const repository = this.parseLink(item.repo.name);
		...
		ReleaseEvent: (item) => {
			...
			const action =
				type === "published"
					? `${Emojis.ReleaseEventCreated} ${localeAction}`
					: null;
			const line = `${action} ${origin} in ${repository}`;
			return line;
		},
	...
	private localeUpperCase = (str: string) =>
		...
		if (typeof item === "object") {
			return "comment" in item.payload
				...
				: "push_id" in item.payload
					? `[${item.payload.head.slice(0, 7)}](${BaseUrl}/${item.repo.name}/commit/${item.payload.head})`
					: "ref" in item.payload
